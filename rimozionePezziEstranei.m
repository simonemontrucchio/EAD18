...
%     title(sprintf('audio tentativo %d, da %f a %f', pos, i/fs_audio, (i+ext)/fs_audio))
    ...
    %vettore differenza tra riferimento e tentativo, e somma del vettore in    %valore assoluto
...
fine = datestr(now);
sprintf('Fatti %d tentativi di rimozione di un pezzo estraneo tra %s e %s', pos-1, inizio, fine) 
